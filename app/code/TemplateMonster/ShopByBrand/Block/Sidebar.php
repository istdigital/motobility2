...
class Sidebar extends Template
{
    ...
    */
    ...
    public function getCategories($sorted = false, $asCollection = false, $toLoad = true)
    {
        ...
        ->getCollection()
        ->addAttributeToSelect("*")
        ->addAttributeToFilter("level",['eq' => 2]);
		
		$categoryDepthLevel = 4;
        ...
        if ( $category->hasChildren() )
        {
            ...
            if ( count($childCategories) > 0 )
            {
                ...
                foreach ( $childCategories as $childCategory )
                {
                    ...
                    ->load($childCategory->getId());
                    if($cat_obj->getProductCollection()->count()>0)
                    {

                    $html .= '<li class="level' . $level . ($this->isActive($childCategory) ? ' active' : '') . '">';
                    $html .= '<a href="' . $this->getCategoryUrl($childCategory) . '?cat='.$childCategory->getId().'" title="' . $childCategory->getName() . '" class="' . ($this->isActive($childCategory) ? 'active' : '') . '">' . $childCategory->getName() .'<span>'.$cat_obj->getProductCollection()->count().'</span></a>';
    ...
    public function isActive($category)
    {
        ...
        /**/
            ...
            ->load($category->getId());
        /**/
        $childrenIds = $cat_obj->getAllChildren(true);
